<script lang="ts">
	...
	import { ADJECTIVE_TYPE_PRETTY_STRING, JLPT_LEVEL_PRETTY_STRING } from "$lib/strings"
...
<ItemPage item={adjective}>
	...
	<Labeled
		...
		class="text-center"
	>
		{#if adjectiveType !== undefined}
			<p>{ADJECTIVE_TYPE_PRETTY_STRING[adjectiveType]}</p>
		{:else}
			<p class="text-neutral-400">-</p>
		{/if}
	</Labeled>
	...
	{#snippet buttons()}
		<Button
			color="secondary"
			href={resolve("/items/edit/[itemId]", { itemId: adjective.id })}
		>
			<EditIcon /> Edit
		</Button>
	{/snippet}
